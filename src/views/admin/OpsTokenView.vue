...
import { isValidTokenName, isValidTokenValue } from '../../utils/validators'
...
const submitForm = async () => {
  try {
    const tokenName = form.value.tokenName.trim()
    ...
    if (!isValidTokenName(tokenName)) {
      message.error('令牌名称支持中英文、数字、_、-，长度 2-50')
      return
    }
    if (formMode.value === 'create') {
      ...
      if (!isValidTokenValue(tokenValue)) {
        message.error('令牌格式不正确（16-128 位，仅字母/数字/._-）')
        return
      }
      ...
      if (tokenValue && !isValidTokenValue(tokenValue)) {
        message.error('令牌格式不正确（16-128 位，仅字母/数字/._-）')
        return
      }
