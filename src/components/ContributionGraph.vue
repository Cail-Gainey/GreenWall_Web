...
import { isValidRepoFullName, isValidRepoName } from '../utils/validators';
...
const confirmPush = async () => {
  ...
  if (pushMode.value === 'create') {
    ...
    if (!isValidRepoName(repoNameValue)) {
      pushError.value = '仓库名称仅支持字母、数字、.、_、-';
      return;
    }
    repoName.value = repoNameValue;
  } else {
    const repoFullNameValue = repoFullName.value.trim();
    if (!repoFullNameValue) {
      pushError.value = '请输入仓库全名（owner/repo）';
      return;
    }
    if (!isValidRepoFullName(repoFullNameValue)) {
      pushError.value = '仓库全名格式应为 owner/repo';
      return;
    }
    repoFullName.value = repoFullNameValue;
  }
  const token = githubProfile.value?.accessToken;
  if (!token) {
